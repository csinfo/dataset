...
Vagrant.configure(2) do |config|
  ...
  config.vm.box = "dennisli1/frapbox"
  ...
  config.vm.define :server do |server|
    server.vm.provider "virtualbox" do |vb|
     ...
     vb.cpus = 2
     # Customize VM name
     vb.name = "CamFlow-server-2nd"
    end
    server.vm.network "private_network", ip: "192.168.33.20"
    server.vm.provision "shell", path: "provision/tomcat_prep.sh"
    server.vm.provision "shell", path: "provision/config_camflow.sh"
    server.vm.provision :unix_reboot
    server.vm.provision "shell", path: "provision/server-post-provision.sh"
  end
